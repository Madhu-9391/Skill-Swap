// src/pages/admin/EngagementAnalytics.jsx
...
const UserCard = ({ user, rank }) => {
  ...
  // Build profile picture URL safely
  const profileURL = profilePicture
    ? `${API_BASE_URL}/uploads/profile-pictures/${profilePicture}`
    : defaultAvatar;
...
const EngagementAnalytics = () => {
  ...
  return (
    <div className="p-6">
      ...
      {users.length > 0 ? (
        users.map((user, idx) => (
          <UserCard key={user._id || idx} user={user} rank={idx} />
        ))
      ) : (
